...
if ( $user->ID ) {
	...
	if ( get_option('comment_registration') || 'private' == $status->post_status )
		err(__('对不起，你必须登录后发表评论.'));	// 將 wp_die 改為錯誤提示
}
...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
        err( __('错误：请填写正确的（姓名和电子邮件）.')); // 將 wp_die 改為錯誤提示
...
?>
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="comment-author vcard">
			...
			<?php printf( __( '<cite class="fn">%s</cite>'), get_comment_author_link() ); ?>:<?php edit_comment_link( __( '编辑' ), ' ' ); ?>
		</div>
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<span style="color:#C00; font-style:inherit">您的评论正在排队XXOO中...</span>
			<br />
			
		<?php endif; ?>
		<div class="comment-body"><?php comment_text(); ?></div>
<span class="commentmetadata"><?php comment_date('Y-m-d') ?> <?php comment_time() ?></span>
	</div>